...
async def test_send_simple_message(message_service):
    ...
    await message_service.send_simple_message(stream_id, message)

    message_service.send_message.assert_called_once_with(stream_id, message, "", "", [], [])
...
async def test_send_complex_message(message_service):
    ...
    await message_service.send_complex_message(stream_id, message)

    message_service.send_message.assert_called_once_with(stream_id, content, data, version, [attachment], [preview])
...
async def test_blast_simple_message(message_service):
    ...
    message_service.blast_message = AsyncMock(return_value=blast_message_response)
    stream_ids = ["sid1", "sid2"]
    message = "<messageML>Hello</messageML>"

    await message_service.blast_simple_message(stream_ids, message)

    message_service.blast_message.assert_called_once_with(stream_ids, message, "", "", [], [])
...
async def test_blast_complex_message(message_service):
    ...
    await message_service.blast_complex_message(stream_ids, message)

    message_service.blast_message.assert_called_once_with(stream_ids, content, data, version, [attachment], [preview])
